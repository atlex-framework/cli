...
├── routes/
│   ├── web.ts               — Web routes
│   ├── api.ts               — API routes mounted under /api
...
Open `routes/web.ts`. This file handles web/root routes like `/`:
...
API routes belong in `routes/api.ts`, which is preloaded by `main.ts`. Routes
declared inside the default `Route.group('/api', ...)` are mounted under `/api`:
...
Route.group('/api', () => {
  Route.get('/users', (_req: Request, res: Response) => {
    res.json({ users: [] })
  })
})
```
